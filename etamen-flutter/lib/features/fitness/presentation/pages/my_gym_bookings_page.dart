...
class MyGymBookingsPage extends ConsumerWidget {
  const MyGymBookingsPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(gymBookingsControllerProvider);
    final controller = ref.read(gymBookingsControllerProvider.notifier);

    return AppScaffold(
      title: uxCopy(context, 'حجوزات الجيم', 'My gym bookings'),
          ...
          children: [
            if (state.isLoading)
              ...
              EmptyView(
                message: uxCopy(
                  context,
                  'لا توجد حجوزات جيم بعد.',
                  'No gym bookings yet.',
                ),
                icon: Icons.fitness_center_outlined,
              )
            else
              ...state.items.map((booking) => _GymBookingCard(booking)),
          ],
        ),
      ),
    );
...
class _GymBookingCard extends StatelessWidget {
  const _GymBookingCard(this.booking);
    ...
    return SoftMedicalCard(
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () =>
                context.push(RouteNames.gymBookingDetails(booking.id)),
